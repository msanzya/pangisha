-- Migration script to extend Pangisha database for agent onboarding and wallet system

-- Add agent_id to properties table to link properties to agents
ALTER TABLE properties 
ADD COLUMN agent_id INT NULL AFTER landlord_id,
ADD FOREIGN KEY (agent_id) REFERENCES agents(id);

-- Add property_id and agent_id to tenants table for linking
ALTER TABLE tenants 
...
CREATE TABLE IF NOT EXISTS agents_wallet_transactions (
    ...
    reference_id INT NULL, -- Reference to payment or contract ID
    reference_type VARCHAR(50) NULL, -- 'payment', 'contract', etc.
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (agent_id) REFERENCES agents(id)
);
